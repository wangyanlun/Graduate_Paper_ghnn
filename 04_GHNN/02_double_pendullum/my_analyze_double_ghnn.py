...
plt.plot(x2_pred[train_mask], y2_pred[train_mask], lw=2.2, color=c_pred, label='GHNN (t<=5)')
plt.plot(x2_pred[~train_mask], y2_pred[~train_mask], lw=2.2, color=c_pred, ls='--', label='GHNN (t>5)')
plt.xlabel("x2")
plt.ylabel("y2")
plt.title("End-Bob Trajectory (Double Pendulum) - Traj 0 (GHNN)")
plt.legend()
plt.savefig('Results/DoublePendulum_GHNN/xy_traj_0.png', dpi=200)
plt.close()
...
for traj in example_trajs:
    ...
    fig.suptitle(f"Energy vs Time (Double Pendulum) - Traj {traj} (GHNN)")
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(f'Results/DoublePendulum_GHNN/energy_traj_{traj}.png', dpi=200)
    plt.close()
...
plt.title("GHNN MAE vs Time (Double Pendulum)")
plt.legend()
plt.savefig('Results/DoublePendulum_GHNN/mae_vs_time.png', dpi=200)
plt.close()

print("XY plot: Results/DoublePendulum_GHNN/xy_traj_0.png")
print("Energy plots: Results/DoublePendulum_GHNN/energy_traj_[traj].png")
print("MAE plot: Results/DoublePendulum_GHNN/mae_vs_time.png")