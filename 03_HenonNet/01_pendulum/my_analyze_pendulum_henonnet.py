...
# 1. Phase Space (p-q with correct shading)
...
for k, traj in enumerate(example_trajs):
    ...
    ax.plot(pred['q_pred'], pred['p_pred'], '--', lw=2, label=f'Traj {traj} HenonNet' if k==0 else None, color=f'C{k}', alpha=0.7)
ax.set_xlabel("q (angle)")
ax.set_ylabel("p (momentum)")
ax.set_title("Phase Space: True vs. HenonNet Prediction\n(Gray = Unseen region, White = Training region)")
ax.legend()
plt.savefig('Results/Pendulum_HENONNET/phase_space.png', dpi=150)
plt.close()

# 2. Energy Conservation Plot (normalized time axis)
for k, traj in enumerate(example_trajs):
    ...
    plt.plot(t_norm, total_h(q_pred, p_pred), label="HenonNet H", color='C0', alpha=0.5)
    plt.plot(t_norm, kinetic(p_pred), label='HenonNet K', ls='--', color='C1', alpha=0.5)
    plt.plot(t_norm, potential(q_pred), label='HenonNet V', ls='-', color='C2', alpha=0.5)
