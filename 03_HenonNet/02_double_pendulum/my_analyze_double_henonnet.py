...
plt.plot(x2_pred[train_mask], y2_pred[train_mask], lw=2.2, color=c_pred, label='HenonNet (t<=5)')
plt.plot(x2_pred[~train_mask], y2_pred[~train_mask], lw=2.2, color=c_pred, ls='--', label='HenonNet (t>5)')
plt.xlabel("x2")
plt.ylabel("y2")
plt.title("End-Bob Trajectory (Double Pendulum) - Traj 0 (HenonNet)")
plt.legend()
plt.savefig('Results/DoublePendulum_HENONNET/xy_traj_0.png', dpi=200)
plt.close()
...
for traj in example_trajs:
    ...
    t = merged['t'].values
    K_true, V_true, H_true = energy_terms(merged['q1'].values, merged['q2'].values, merged['p1'].values, merged['p2'].values)
    K_pred, V_pred, H_pred = energy_terms(merged['q1_pred'].values, merged['q2_pred'].values, merged['p1_pred'].values, merged['p2_pred'].values)
    ...
    fig.suptitle(f"Energy vs Time (Double Pendulum) - Traj {traj} (HenonNet)")
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(f'Results/DoublePendulum_HENONNET/energy_traj_{traj}.png', dpi=200)
    plt.close()
...
plt.title("HenonNet MAE vs Time (Double Pendulum)")
plt.legend()
plt.savefig('Results/DoublePendulum_HENONNET/mae_vs_time.png', dpi=200)
plt.close()

print("XY plot: Results/DoublePendulum_HENONNET/xy_traj_0.png")
print("Energy plots: Results/DoublePendulum_HENONNET/energy_traj_[traj].png")
print("MAE plot: Results/DoublePendulum_HENONNET/mae_vs_time.png")