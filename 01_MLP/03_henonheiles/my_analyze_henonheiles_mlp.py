...
plt.xlabel("x")
plt.ylabel("y")
plt.title("Trajectory (Henon-Heiles) - Traj 0")
plt.legend()
...
for traj in example_trajs:
    ...
    fig.suptitle(f"Energy vs Time (Henon-Heiles) - Traj {traj}")
...
plt.plot(mae_by_time_df['t'].values, mae_by_time_df['mae_row'].values, color=c_true, lw=2.2, label="MLP MAE")
plt.axvline(10.0, color='k', ls=':', lw=2, label='Train/Test split (t=10)')
plt.xlabel("Time t")
plt.ylabel("Mean Abs Error (x,y,px,py)")
plt.title("MLP MAE vs Time (Henon-Heiles)")
plt.legend()
